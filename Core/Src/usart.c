...
uint8_t g_usart2_rx_buf[USART2_REC_LEN];  // USART2 接收缓存
uint16_t g_usart2_rx_sta = 0;             // USART2 状态字
uint8_t g_rx2_buffer[RXBUFFERSIZE1]; 
/**
 * @brief       串口数据接收回调函数
                数据处理在这里进行		/接收格式 123\r\n
 * @param       huart:串口句柄
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        if ((g_usart2_rx_sta & 0x8000) == 0)            
        {
            if (g_usart2_rx_sta & 0x4000)                
            {
...
void USART2_IRQHandler(void)
{
    ;                               /*  */
		HAL_UART_IRQHandler(&huart2);  
		 ...
		 while (HAL_UART_Receive_IT(&huart2, (uint8_t *)g_rx2_buffer, RXBUFFERSIZE1) != HAL_OK)     /* */
    {
        /*  */
    }
}
